...
<head>
    ...
    <script> // Restore Bootstrap carousel sliding and show prev/next cards with scale effect
    ...
    <div class="container">
        ...
        <div class="poll-dates">
            <div class="row">
                <div class="col-md-6">
                    ...
                    <p class="mb-0"><strong>Start:</strong>
                        {{ $poll->start_at ? \Carbon\Carbon::parse($poll->start_at)->format('M d, Y h:i A') : 'N/A' }}
                    </p>
                    <p class="mb-0"><strong>End:</strong>
                        {{ $poll->end_at ? \Carbon\Carbon::parse($poll->end_at)->format('M d, Y h:i A') : 'N/A' }}</p>
                </div>
                <div class="col-md-6">
        ...
        <div class="winner-summary">
            ...
            <div class="row">
                ...
                $highestVotes = $nominees->max('vote_count');
                $lowestVotes = $nominees->min('vote_count');
                $winners = $nominees->where('vote_count', $highestVotes);
                } else {
                $highestVotes = 0;
                $lowestVotes = 0;
                $winners = collect();
                ...
                <div class="col-md-10 mb-3">
                    ...
                    @if($nominees->isEmpty())
                    <span class="text-muted">No nominees</span>

                    @elseif($highestVotes == 0)
                    <span class="text-muted">No votes yet</span>

                    @elseif($highestVotes === $lowestVotes)
                    <span class="badge bg-info">
                        All nominees are tied ({{ $highestVotes }} votes each)
                    </span>

                    @elseif($winners->count() > 1)
                    <span class="badge bg-warning text-dark">
                        Currently tied:
                        {{ $winners->pluck('name')->implode(', ') }}
        ...
        <div class="category-card">
            ...
            <div class="nominee-carousel-container">
                <div class="custom-nominee-carousel" id="custom-carousel-{{ $category->id }}"
                    style="overflow: hidden; position: relative; width: 100%;">
                    <div class="custom-carousel-track d-flex align-items-center justify-content-center"
                        style="transition: transform 0.7s cubic-bezier(.4,2,.6,1); will-change: transform;">
                        @php
                        $original = $categoryNominees;
                        $repeatCount = 5; // enough to simulate infinity
                        $cards = collect();

                        for ($i = 0; $i < $repeatCount; $i++) { foreach ($original as $nominee) { $cards->
                            push($nominee);
                            }
                            }
                            @endphp
                            ...
                            <div class="profile-card custom-carousel-card my-5" data-index="{{ $i }}"
                                style="min-width: 320px; max-width: 340px; margin: 0 1rem;">
                                @if($nominee->photo)
                                ...
                                <span class="badge badge-pill badge-success"
                                    style="font-size:1.1em;">{{ $nominee->vote_count }} votes</span>
                    ...
                    <button class="btn btn-link custom-carousel-prev"
                        style="position:absolute;left:0;top:50%;transform:translateY(-50%);z-index:10;font-size:2rem;color:#fff;"><i
                            class="fas fa-chevron-left text-warning"></i></button>
                    <button class="btn btn-link custom-carousel-next"
                        style="position:absolute;right:0;top:50%;transform:translateY(-50%);z-index:10;font-size:2rem;color:#fff;"><i
        ...
        document.querySelectorAll('.custom-nominee-carousel').forEach(carousel => {
            ...
            function update(animate = true) {
                ...
                cards.forEach((c, i) => {
                    ...
                    c.style.transform = d === 0 ? 'scale(1.08)' : d === 1 ? 'scale(0.95)' :
                        'scale(0.85)';
